...
class TpchSchemaProvider(sc: SparkContext, inputDir: String) {
  ...
  val dfMap = Map(
    "customer" -> sqlContext.read.parquet(inputDir + "/customer.parquet"),

    "lineitem" -> sqlContext.read.parquet(inputDir + "/lineitem.parquet"),

    "nation" -> sqlContext.read.parquet(inputDir + "/nation.parquet"),

    "region" -> sqlContext.read.parquet(inputDir + "/region.parquet"),

    "order" -> sqlContext.read.parquet(inputDir + "/orders.parquet"),

    "part" -> sqlContext.read.parquet(inputDir + "/part.parquet"),

    "partsupp" -> sqlContext.read.parquet(inputDir + "/partsupp.parquet"),

    "supplier" -> sqlContext.read.parquet(inputDir + "/supplier.parquet"))
